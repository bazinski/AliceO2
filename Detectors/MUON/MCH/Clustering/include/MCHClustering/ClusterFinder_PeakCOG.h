...
#include "VirtualClusterFinder.h"
...
class ClusterFinder_PeakCOG : public VirtualClusterFinder
{
public:
  AliMUONClusterFinderPeakCOG(bool plot, VirtualClusterFinder* clusterFinder); // Constructor
...
private:
  /// Not implemented
  ClusterFinder_PeakCOG(const AliMUONClusterFinderPeakCOG& rhs);
  /// Not implemented
  ClusterFinder_PeakCOG& operator=(const AliMUONClusterFinderPeakCOG& rhs);
...
private:
  ...
  TH2D *mHistAnode; //!<! histo for peak search
  ...
  ClassDef(ClusterFinder_PeakCOG,0) // cluster finder in MUON arm of ALICE
};
