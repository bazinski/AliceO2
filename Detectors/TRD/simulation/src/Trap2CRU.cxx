...
int Trap2CRU::buildDigitRawData(const int digitstartindex, const int digitendindex, const int mcm, const int rob, const uint32_t triggerrecordcount)
{
  ...
  LOG(debug) << "raw pointer at end : " << (void*)mRawDataPtr;
  LOG(debug) << __func__ << " leaving with digitswritten of: " << digitwordswritten << " for det:rob:mcm of " << startdet << ":" << startrob << ":" << startmcm;
...
//  }
  ...
  while (linkid == mTracklets[trackletindex + trackletcounter].getHCID() && header.col == mTracklets[trackletindex + trackletcounter].getColumn() && header.padrow == mTracklets[trackletindex + trackletcounter].getPadRow()) {
    ...
    if(headerqpart==0xff){  
      LOG(info) <<  mTracklets[trackletindex + trackletcounter].getQ2() << 2 +
        ((mTracklets[trackletindex + trackletcounter].getQ1()) >> 5)
    ...
    switch (trackletcounter) {
      ...
      case 1:
        ...
        if(header.pid1==0xff ) LOG(warn) << "ZZ we are setting pid1 but pid0 is not set, a second tracklet but no first one?";
        break;
      case 2:
        header.pid2 = headerqpart;
        if(header.pid1==0xff || header.pid0==0xff) LOG(warn) << "ZZ we are setting pid2 but pid0/1 is not set, a second tracklet but no first one?" << header.pid0 << " " << header.pid1;
        break;
      default:
      ...
      if (mCurrentTracklet < mTracklets.size()) {
        ...
        if (mCurrentDigit < mDigits.size()) {
          while (isDigitOnLink(linkid, mCurrentDigit)) {
            LOG(debug) << "digit is on link for linkid : " << linkid << " and digit index of : " << mCurrentDigit;
            if (trackletcounter == 0) {
