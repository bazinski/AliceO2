...
void TrapSimulator::init(TrapConfig* trapconfig, int det, int robPos, int mcmPos)
{
  ...
  //
  //
  LOG(info) << " : trap sim is at : 0x" << hex << trapconfig;
  if (!mInitialized) {
...
bool TrapSimulator::checkInitialized() const
{
  ...
  //  if (!mInitialized)
  //   LOG(info) << "TrapSimulator is not initialized but function other than Init() is called.";
...
void TrapSimulator::print(int choice) const
{
  ...
  if ((choice & PRINTFOUND) != 0) {
    ...
    for (int iTrkl = 0; iTrkl < mTrackletArray64.size(); iTrkl++) {
      LOG(info) << "tracklet " << iTrkl << ": 0x" << hex << std::setw(32) << mTrackletArray64[iTrkl].getTrackletWord();
      LOG(info) << mTrackletArray64[iTrkl];
    }
  }
}
...
void TrapSimulator::draw(int choice, int index)
{
  ...
  if ((choice & PLOTTRACKLETS) != 0) {
    ...
    for (int iTrkl = 0; iTrkl < mTrackletArray64.size(); iTrkl++) {
      ...
      LOG(info) << "Tracklet " << iTrkl << ": y = " << trkl.getPosition() << ", slope = " << (float)trkl.getSlope() << "for a det:rob:mcm combo of : " << mDetector << ":" << mRobPos << ":" << mMcmPos;
      ...
      LOG(info) << "Tracklet " << iTrkl << ": t0 : " << t0 << ", t1 " << t1 << ", slope:" << slope << ",  which comes from : " << mTrapConfig->getDmemUnsigned(mgkDmemAddrNdrift, mDetector, mRobPos, mMcmPos) << " shifted 5 to the right ";
...
void TrapSimulator::setData(int adc, const ArrayADC& data, std::vector<o2::MCCompLabel>& labels)
{
  ...
  //  LOG(info) << "setting data labels incoming of  : " << labels.size() << " with adc of " << adc;
  mADCLabels[adc] = labels;
  //  LOG(info) << "setting data labels incoming of  : " << labels.size() << " with adc of " << adc << " now mADCLabels[adc] size is : " << mADCLabels[adc].size();
}
...
int TrapSimulator::packData(std::vector<uint32_t>& rawdata, uint32_t offset)
{
  ...
  LOG(info) << "packing data with trackletarry64 size of : " << mTrackletArray64.size();
  for (int i = 0; i < 3; i++) {
    if (i < mTrackletArray64.size()) { // we have  a tracklet
      LOG(info) << "we have a tracklet at i=" << i << " with trackletword 0x" << mTrackletArray64[i].getTrackletWord();
      switch (i) {
        case 0:
      ...
      switch (i) {
        case 1:
          mcmhead.pid1 = 0xff;
          LOG(info) << "setting mcmhead pid1 to 0xff with tracklet array size of " << mTrackletArray64[i];
  ...
  LOG(info) << "pushing back mcm head of 0x" << std::hex << mcmhead.word << " with trackletcount of : " << std::dec << trackletcount << ":-:" << wordswritten;
  rawdata.push_back(mcmhead.word); //memcpy(&rawdata[wordswritten++], &mcmhead, sizeof(mcmhead));
  wordswritten++;
  for (int i = 0; i < trackletcount; i++) {
    LOG(info) << "pushing back mcmtrackletword of 0x" << std::hex << tracklets[i].word;
  ...
  if (debugheaders) {
    LOG(info) << ">>>>> START info OUTPUT OF packData trackletcount:-:wordcount" << trackletcount << ":-:" << wordswritten;
    ...
    LOG(info) << "<<<<<  END info OUTPUT OF packData";
  }
  //must produce between 2 and 4 words ... 1 and 3 tracklets.
  //  assert(wordswritten<5);
  //  assert(wordswritten>1);
  LOG(info) << "now to leave pack data after passing asserts with wordswritten = " << wordswritten;
...
void TrapSimulator::addHitToFitreg(int adc, unsigned short timebin, unsigned short qtot, short ypos)
{
  ...
  //
  LOG(info) << __func__ << " adc : " << adc << " timebin: " << timebin << " qtot: " << qtot << " ypos:" << ypos;
  if (adc > 24) {
  ...
  if ((timebin >= mTrapConfig->getTrapReg(TrapConfig::kTPFS, mDetector, mRobPos, mMcmPos)) &&
    ...
    LOG(info) << "fitreg[" << adc << "] in timebin " << timebin << ": X=" << mFitReg[adc].mSumX
...
void TrapSimulator::calcFitreg()
{
  ...
  //??? to be clarified:
  LOG(info) << "ENTERING : " << __FILE__ << ":" << __func__ << ":" << __LINE__ << " :: " << getDetector() << ":" << getRobPos() << ":" << getMcmPos() << " -------------------- mNHits : " << mNHits;
  ...
  for (timebin = timebin1; timebin < timebin2; timebin++) {
    ...
    if (found > 4) // sorting like in the TRAP in case of 5 or 6 candidates!
    {
      ...
      if (worse1 < 19) {
        qTotal[worse1] = 0;
        //LOG(info) << "Kill ch " << worse1;
      }
      if (worse2 < 19) {
        qTotal[worse2] = 0;
        // LOG(info) << "Kill ch " << worse2;
      }
    }
    ...
    for (adcch = 0; adcch < 19; adcch++) {
      if (qTotal[adcch] > 0) // the channel is marked for processing
      {
        ...
        LOG(info) << __func__ << "ADDING HIT FOR in adclabels : " << mADCLabels[adcch].size() << " with adcch of " << adcch << " qtot : " << qTotal[adcch] << " timebin :" << timebin << " and ypos:" << ypos;
...
void TrapSimulator::fitTracklet()
{
  ...
  for (int cpu = 0; cpu < 4; cpu++) {
    if (mFitPtr[cpu] == 31) {
      ...
      // fit0->Print();
      // fit1->Print();
      LOG(info) << "mult is : " << mult << " and in hex : 0x" << std::hex << mult << std::dec;
      mult = 1;
      mult = mult << (32 + decPlaces);
      mult = -mult;
      LOG(info) << "after mult is : " << mult << " and in hex : 0x" << std::hex << mult << std::dec;
...
int TrapSimulator::getPID(int q0, int q1)
{
  ...
  if (addrQ0 >= nBinsQ0) { // check for overflow
    //    LOG(info) << "Overflow in q0: " << addrQ0 << "/4 is bigger then " << nBinsQ0;
    addrQ0 = nBinsQ0 - 1;
  }
  ...
  if (addr >= pidTotalSize) {
    //  LOG(info) << "Overflow in q1. Address " << addr << "/4 is bigger then " << pidTotalSize;
    addr = pidTotalSize - 1;
  }
