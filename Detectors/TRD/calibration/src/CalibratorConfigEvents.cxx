      ...
      if (mcmid < constants::MAXMCMCOUNT) {
        ...
        if (mTrapRegistersFrequencyMap[mcmid][mcmreg].size() > 1) {
          ...
          for (auto& apair : mTrapRegistersFrequencyMap[mcmid][mcmreg]) {
            LOGP(debug, "XXX  [{}] GREATER than 1 value for mcmid #{} mcm register : {}[{},{}] value : {} count 1=={} ", count++, mcmid, mCCDBObject.getRegNameByIdx(mcmreg), mCCDBObject.getRegAddrByIdx(mcmreg), mcmreg, apair.first, mTrapRegistersFrequencyMap[mcmid][mcmreg].size());
          }
        }
      } // if(mcmid<constants::MAXMCMCOUNT)
      else {
...
void CalibratorConfigEvents::collapseRegisterValues()
{
  ...
  for (uint32_t mcmid = 0; mcmid < constants::MAXMCMCOUNT; ++mcmid) {
    ...
    if (mCCDBObject.isMCMPresent(mcmid)) {
      ...
      // TODO mcm being present in the ccdbobject is wrong, it must be added if it is in the frequencymap but not the ccdbobject mcmpresent vector.
      //
      //
