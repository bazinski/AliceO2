...
DataProcessorSpec getTRDConfigEventCalibSpec()
{
  ...
  return DataProcessorSpec{
    ...
    inputs,
    outputs,
//yyp    AlgorithmSpec{adaptFromTask<device>(ccdbRequest)},
    AlgorithmSpec{adaptFromTask<device>(ConfigEventCalibDevice)},//TRDConfigEventCalibSpec)},
    Options{
      {"min-to-accumulate", VariantType::Int, 15, {"time to accumulate config events before we check ccdb for a difference."}},
