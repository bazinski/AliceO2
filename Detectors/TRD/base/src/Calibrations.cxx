...
void Calibrations::setCCDB(int calibrationobjecttype, long timestamp)
{
  ...
  switch (calibrationobjecttype) {
    case 1: //simulation
      ...
      if(!mChamberCalibrations) LOG(fatal) << "No chamber calibrations returned from CCDB for TRD calibrations,  or TRD from what ever you are running.";
      mLocalVDrift = ccdbmgr.get<o2::trd::LocalVDrift>("TRD_test/LocalVDrift");
      if(!mLocalVDrift) LOG(fatal) << "No Local V Drift calibrations returned from CCDB for TRD calibrations,  or TRD from what ever you are running.";
      mLocalT0 = ccdbmgr.get<o2::trd::LocalT0>("TRD_test/LocalT0");
      if(!mLocalT0) LOG(fatal) << "No Local T0 calibrations returned from CCDB for TRD calibrations,  or TRD from what ever you are running.";
      ...
      if(!mPadNoise) LOG(fatal) << "No Padnoise calibrations returned from CCDB for TRD calibrations,  or TRD from what ever you are running.";
      mChamberStatus = ccdbmgr.get<o2::trd::ChamberStatus>("TRD_test/ChamberStatus");
      if(!mChamberStatus) LOG(fatal) << "No ChamberStatus calibrations returned from CCDB for TRD calibrations,  or TRD from what ever you are running.";
      mPadStatus = ccdbmgr.get<o2::trd::PadStatus>("TRD_test/PadStatus");
      if(!mPadStatus) LOG(fatal) << "No Pad Status calibrations returned from CCDB for TRD calibrations,  or TRD from what ever you are running.";
      mChamberNoise = ccdbmgr.get<o2::trd::ChamberNoise>("TRD_test/ChamberNoise");
      if(!mChamberNoise) LOG(fatal) << "No ChamberNoise calibrations returned from CCDB for TRD calibrations,  or TRD from what ever you are running.";
