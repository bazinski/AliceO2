...
TrapConfigEventParser::TrapConfigEventParser()
{
  ...
  mTrapConfigEvent = std::make_unique<o2::trd::TrapConfigEvent>(); // std::make_shared<TrapConfigEvent>();
    ...
    if (mCurrentMCMID < o2::trd::constants::MAXMCMCOUNT && mRegistersReadForCurrentMCM - 1 < TrapConfigEvent::kLastReg) {
      ...
      if (mRegistersReadForCurrentMCM > 0) {
        ...
        if (registerdata > regmax) {
          LOGP(debug, "assumed corrupted data as register data is greater than the mask : {:08x} for max {:08x} registername : {} regaddress : {}", registerdata, regmax, mTrapConfigEvent.get()->getRegNameByIdx(mRegistersReadForCurrentMCM - 1), registeraddr);
          // TODO put into QC
      ...
      if (mCurrentMCMID < o2::trd::constants::MAXMCMCOUNT && mRegistersReadForCurrentMCM < TrapConfigEvent::kLastReg) {
        ...
        if (mRegistersReadForCurrentMCM > 0) {
          ...
          if (registerdata <= regmax) {
            ...
            LOGP(warn, "{} assumed corrupted data as register data is greater than the mask : {:08x} for max {:08x} registername : {} regaddress : {}", __LINE__, registerdata, regmax, mTrapConfigEvent.get()->getRegNameByIdx(mRegistersReadForCurrentMCM - 1), registeraddr);
          }
        }
      } else {
