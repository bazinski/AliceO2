    ...
    if (currentlinksize32 > 0) { // if link is not empty
      ...
      *****************/
      ...
      if (mHBFoffset32 != endOfCurrentLink &&
        ...
        if (mDigitHCHeader.major == 0x47 || mDigitHCHeader.major == 0x1) {
          // config event so ignore for now and bail out of parsing.
          //https://alice.physi.uni-heidelberg.de/viewvc/trd/TRAPconfig/trunk/src/common/pack_conf.asm
          //https://alice.physi.uni-heidelberg.de/viewvc/trd/TRAPconfig/trunk/src/common/pack_conf_inc.asm
          ...
          if (mFirstConfigIR) {
            mLastConfigIR = mIR;
            mFirstConfigIR = false;
          }
          timediff = mLastConfigIR.differenceInBCMUS(mIR);
          if (timediff != 0) {
            //fill histograms
            LOGP(info, "New Config event for : on hcid : {}  link: {} -- Difference from last IR : {}us bc: {} orbit:{} eventnum {} timediff : {} Trackletwords : {}", halfChamberId, currentlinkindex, timediff, mIR.bc, mIR.orbit, mConfigEventCount, timediff, trackletWordsRead);
            if (mConfigEventCount < 100) {
              LOGP(info, "Filling histograms for event {}", mConfigEventCount);
              mTrapConfigParser.FillHistograms(mConfigEventCount);
            }
            mTrapConfigParser.clear();
            mConfigEventCount++;
            mTotalConfigTime = (std::chrono::duration<double, std::micro>)0; // configparsingtime = std::chrono::high_resolution_clock::now() - configparsingstart;
            LOG(info) << "New config events";
            mLastConfigIR = mIR;
            mTrapConfigParser.writeFile(mConfigEventCount);
          }
          ...
          LOGP(info, "Config parsing took {} total time for this config events {}", (double)std::chrono::duration_cast<std::chrono::microseconds>(configparsingtime).count(), (double)mTotalConfigTime.count());
        } else {
