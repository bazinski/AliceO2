...
void CruRawReader::incrementErrors(int error, int halfChamberId)
{
  mEventRecords.incParsingError(error, halfChamberId);
  if (mOptions[TRDVerboseErrorsBit]) {
    LOG(info) << "PEPE Parsing Error : " << o2::trd::ParsingErrorsString[error]
              << " sector:stack:layer:side :: "
              << HelperMethods::getSector(halfChamberId / 2) << ":"
              << HelperMethods::getStack(halfChamberId / 2) << ":"
              << HelperMethods::getLayer(halfChamberId / 2) << ":"
              << halfChamberId % 2;
  }
}
...
bool CruRawReader::checkRDH(const o2::header::RDHAny* rdh)
{
  ...
  if (feeid.supermodule > 17) {
    ...
    if (mOptions[TRDVerboseErrorsBit]) {
      LOG(error) << "failed due to supermodule :  " << std::dec << (int)feeid.supermodule << " whole feeid : " << std::hex << (unsigned int)feeid.word;
    }
    incrementErrors(TRDFEEIDBadSector);
    return false;
  }
  if (o2::raw::RDHUtils::getMemorySize(rdh) <= 0) {
    ...
    incrementErrors(TRDParsingBadRDHMemSize);
    return false;
  }
  return true;
}
...
bool CruRawReader::compareRDH(const o2::header::RDHAny* rdhPrev, const o2::header::RDHAny* rdhCurr)
{
  if (o2::raw::RDHUtils::getFEEID(rdhPrev) != o2::raw::RDHUtils::getFEEID(rdhCurr)) {
    ...
    incrementErrors(TRDParsingBadRDHFEEID);
    return false;
  }
  if (o2::raw::RDHUtils::getEndPointID(rdhPrev) != o2::raw::RDHUtils::getEndPointID(rdhCurr)) {
    ...
    incrementErrors(TRDParsingBadRDHEndPoint);
    return false;
  }
  if (o2::raw::RDHUtils::getTriggerOrbit(rdhPrev) != o2::raw::RDHUtils::getTriggerOrbit(rdhCurr)) {
    ...
    incrementErrors(TRDParsingBadRDHOrbit);
    return false;
  }
  if (o2::raw::RDHUtils::getCRUID(rdhPrev) != o2::raw::RDHUtils::getCRUID(rdhCurr)) {
    ...
    incrementErrors(TRDParsingBadRDHCRUID);
    return false;
  }
  if (o2::raw::RDHUtils::getPacketCounter(rdhPrev) == o2::raw::RDHUtils::getPacketCounter(rdhCurr)) {
    ...
    incrementErrors(TRDParsingBadRDHPacketCounter);
    return false;
  }
  return true;
}
...
int CruRawReader::processHBFs()
{
  ...
  while ((mHBFoffset32 < ((mTotalHBFPayLoad) / 4))) {
    if (mOptions[TRDVerboseBit]) {
      LOG(info) << "Looping over cruheaders in HBF, loop count " << iteration << " current offset is" << mHBFoffset32 << " total payload is " << mTotalHBFPayLoad / 4 << "  raw :" << mTotalHBFPayLoad;
    }
...
void CruRawReader::checkDigitHCHeader(int halfChamberIdRef)
{
  ...
  if (!mOptions[TRDIgnoreDigitHCHeaderBit]) {
    if (halfChamberIdHeader != halfChamberIdRef) {
      incrementErrors(TRDParsingDigitHCHeaderMismatch, halfChamberIdRef);
      if (mOptions[TRDVerboseErrorsBit]) {
        LOGF(info, "Sector mismatch in Digit HCHeader : currentsector : %d halfChamberIdHeader: %d halfChamberId : %d", mFEEID.supermodule, halfChamberIdHeader, halfChamberIdRef);
      }
    }
  }
}
...
int CruRawReader::parseDigitHCHeader(int hcid)
{
  ...
  if (additionalHeaderWords >= 3) {
    incrementErrors(TRDParsingDigitHeaderCountGT3, hcid);
    if (mMaxWarnPrinted > 0) {
    ...
    if (mOptions[TRDVerboseErrorsBit]) {
      LOG(info) << "Error parsing DigitHCHeader, too many additional words count="
                << additionalHeaderWords << " header:" << std::hex << mDigitHCHeader.word;
    ...
    switch (getDigitHCHeaderWordType(headers[headerwordcount])) {
      case 1: // header header1;
        if (headersfound.test(0)) {
          ...
          if (mOptions[TRDVerboseErrorsBit]) {
            LOG(info) << "We have a >1 Digit HC Header 1  : " << std::hex << " raw: 0x"
                      << headers[headerwordcount];
            printDigitHCHeader(mDigitHCHeader, headers.data());
          }
          incrementErrors(TRDParsingDigitHCHeader1, hcid);
        }
        mDigitHCHeader1.word = headers[headerwordcount];
        headersfound.set(0);
        if (mDigitHCHeader1.res != 0x1) {
          if (mOptions[TRDVerboseErrorsBit]) {
            LOG(info) << "Digit HC Header 1 reserved : 0x" << std::hex << mDigitHCHeader1.res
                      << " raw: 0x" << mDigitHCHeader1.word;
            printDigitHCHeader(mDigitHCHeader, headers.data());
          }
          incrementErrors(TRDParsingDigitHeaderWrong1, hcid);
        }
      ...
      case 2: // header header2;
        if (headersfound.test(1)) {
          ...
          if (mOptions[TRDVerboseErrorsBit]) {
            LOG(info) << "We have a >1 Digit HC Header 2  : " << std::hex << " raw: 0x"
                      << headers[headerwordcount];
            printDigitHCHeader(mDigitHCHeader, headers.data());
          }
        ...
        if (mDigitHCHeader2.res != 0b110001) {
          if (mOptions[TRDVerboseErrorsBit]) {
            LOG(info) << "Digit HC Header 2 reserved : " << std::hex << mDigitHCHeader2.res
                      << " raw: 0x" << mDigitHCHeader2.word;
          }
          incrementErrors(TRDParsingDigitHeaderWrong2, hcid);
        }
        break;
      case 3: // header header3;
        if (headersfound.test(2)) {
          ...
          incrementErrors(TRDParsingDigitHCHeader3, hcid);
        }
        mDigitHCHeader3.word = headers[headerwordcount];
        headersfound.set(2);
        if (mDigitHCHeader3.res != 0b110101) {
          if (mOptions[TRDVerboseErrorsBit]) {
            LOG(info) << "Digit HC Header 3 reserved : " << std::hex << mDigitHCHeader3.res
                      << " raw: 0x" << mDigitHCHeader3.word;
            printDigitHCHeader(mDigitHCHeader, headers.data());
          }
          incrementErrors(TRDParsingDigitHeaderWrong3, hcid);
        }
        if (mPreviousDigitHCHeadersvnver != 0xffffffff &&
          ...
          if ((mDigitHCHeader3.svnver != mPreviousDigitHCHeadersvnver) &&
            ...
            if (mOptions[TRDVerboseErrorsBit]) {
              LOG(info) << "Digit HC Header 3 svn ver : " << std::hex << mDigitHCHeader3.svnver
                        << " svn release ver : 0x" << mDigitHCHeader3.svnrver;
              printDigitHCHeader(mDigitHCHeader, headers.data());
            }
            incrementErrors(TRDParsingDigitHCHeaderSVNMismatch, hcid);
            return -1;
          } else {
      ...
      default:
        ...
        incrementErrors(TRDParsingDigitHeaderWrong4, hcid);
    }
  }
  if (mOptions[TRDVerboseBit]) {
    printDigitHCHeader(mDigitHCHeader, &headers[0]);
...
int CruRawReader::processHalfCRU(int iteration)
{
  ...
  if (iteration > 0) {
    ...
    if (mCurrentHalfCRUHeader.StopBit != mPreviousHalfCRUHeader.StopBit) {
      if (mOptions[TRDVerboseErrorsBit]) {
        LOG(info) << "Error after " << iteration << " cru loops current stopbit : "
                  << mCurrentHalfCRUHeader.StopBit << " previous end point : "
                  << mPreviousHalfCRUHeader.StopBit;
      }
      incrementErrors(TRDParsingHalfCRUCorrupt);
  ...
  if (totalHalfCRUDataLength32 > (mTotalHBFPayLoad / 4) - mHBFoffset32) {
    ...
    if (mOptions[TRDVerboseErrorsBit]) {
      LOG(warn) << "Next HalfCRU header says it contains more data than in the rdh payloads! "
                << totalHalfCRUDataLength32 << " < " << mTotalHBFPayLoad << "-" << mHBFoffset32
                << " sector:side:endpoint: " << (unsigned int)mFEEID.supermodule << ":"
                << (unsigned int)mFEEID.side << ":" << (unsigned int)mFEEID.endpoint;
    }
  ...
  if (!halfCRUHeaderSanityCheck(mCurrentHalfCRUHeader, mCurrentHalfCRULinkLengths,
    ...
    if (mOptions[TRDVerboseErrorsBit]) {
      LOG(info) << "HalfCRU header failed sanity check for FEEID with  sector:side:endpoint: "
                << (unsigned int)mFEEID.supermodule << ":" << (unsigned int)mFEEID.side << ":"
                << (unsigned int)mFEEID.endpoint;
    }
    incrementErrors(TRDParsingHalfCRUCorrupt);
    mWordsRejected += (mTotalHBFPayLoad / 4) - mHBFoffset32;
    return -2;
  ...
  if (o2::ctp::TriggerOffsetsParam::Instance().LM_L0 > (int)mIR.bc) {
    ...
    if (mOptions[TRDVerboseErrorsBit]) {
      LOG(info) << "Bunchcrossing from previous orbit, is negative after shift and data is being rejected LM_L0:" << o2::ctp::TriggerOffsetsParam::Instance().LM_L0 << " bc:" << (int)mIR.bc;
    }
  ...
  for (auto& linkerror : mCurrentHalfCRULinkErrorFlags) {
    if (linkerror != 0) {
      if (mOptions[TRDVerboseBit]) {
        LOG(info) << "E link error FEEID:" << mFEEID.word << " CRUID:" << mCRUID << " Endpoint:"
                  << mCRUEndpoint << " on linkcount:" << linkerrorcounter++ << " errorval:0x"
                  << std::hex << linkerror;
      }
    }
  }
  ...
  for (int currentlinkindex = 0; currentlinkindex < NLINKSPERHALFCRU; currentlinkindex++) {
    ...
    if (linkstart != linkend) { // if link is not empty
      ...
      if (trackletWordsRead == -1) {
        ...
        if (mOptions[TRDVerboseErrorsBit]) {
          LOG(info) << "TrackletParser returned -1 for  LINK # " << currentlinkindex << " an FEEID:"
                    << std::hex << mFEEID.word << " det:" << std::dec << detectorId
                    << " is > the lenght stored in the cruhalfchamber header : "
                    << mCurrentHalfCRULinkLengths[currentlinkindex];
        }
        incrementErrors(TRDParsingTrackletsReturnedMinusOne, halfChamberId);
        return -2;
      }
      ...
      if (mOptions[TRDVerboseBit]) {
        LOG(info) << "trackletwordsread:" << trackletWordsRead << " trackletwordsrejected:"
                  << trackletWordsDumped << " parsing with linkstart: " << linkstart
                  << " ending at : " << linkend;
      }
        ...
        if (mDigitHCHeader.major == 0x47) {
          ...
          if (digitWordsRead + digitWordsRejected != std::distance(linkstart, linkend)) {
            ...
            if (mFixDigitEndCorruption) {
              ...
              incrementErrors(TRDParsingDigitDataStillOnLink, halfChamberId);
              if (mOptions[TRDVerboseErrorsBit]) {
