...
void TrapConfig3::buildIndex()
{
  // to speed things up build an index of the map by address instead of position of
  //for(auto& tuple : TrapRegisterMap){
  //auto name = std::get<>tuple.second
  //  TrapRegisterMap_addr(std::make_tuple(tuple.));
  //original{kSML0, {"SML0", 0x0A00, 15}},
  //new {0x0A00, {"SML0", kSML0, 15}},
  // }
}
void TrapConfig3::initRegs()
{
  ...
  mTrapRegisters[kIA0IRQ0].init("IA0IRQ0", 0x0B00, 15, 83, 0, false, 12);
  mTrapRegisters[kIA0IRQ1].init("IA0IRQ1", 0x0B01, 15, 83, 1, false, 12);
  mTrapRegisters[kIA0IRQ2].init("IA0IRQ2", 0x0B02, 15, 83, 2, false, 12);
  mTrapRegisters[kIA0IRQ3].init("IA0IRQ3", 0x0B03, 15, 83, 3, false, 12);
  mTrapRegisters[kIA0IRQ4].init("IA0IRQ4", 0x0B04, 15, 83, 4, false, 12);
  mTrapRegisters[kIA0IRQ5].init("IA0IRQ5", 0x0B05, 15, 83, 5, false, 12);
  mTrapRegisters[kIA0IRQ6].init("IA0IRQ6", 0x0B06, 15, 83, 6, false, 12);
  mTrapRegisters[kIA0IRQ7].init("IA0IRQ7", 0x0B07, 15, 83, 7, false, 12);
  mTrapRegisters[kIA0IRQ8].init("IA0IRQ8", 0x0B08, 15, 83, 8, false, 12);
  mTrapRegisters[kIA0IRQ9].init("IA0IRQ9", 0x0B09, 15, 83, 9, false, 12);
  mTrapRegisters[kIA0IRQA].init("IA0IRQA", 0x0B0A, 15, 83, 10, false, 12);
  mTrapRegisters[kIA0IRQB].init("IA0IRQB", 0x0B0B, 15, 83, 11, false, 12);
  mTrapRegisters[kIA0IRQC].init("IA0IRQC", 0x0B0C, 15, 83, 12, false, 12);
  mTrapRegisters[kIRQSW0].init("IRQSW0", 0x0B0D, 15, 83, 13, false, 13);
  mTrapRegisters[kIRQHW0].init("IRQHW0", 0x0B0E, 15, 83, 14, false, 13);
  mTrapRegisters[kIRQHL0].init("IRQHL0", 0x0B0F, 15, 83, 15, false, 13);
  mTrapRegisters[kIA1IRQ0].init("IA1IRQ0", 0x0B20, 15, 91, 0, false, 12);
  mTrapRegisters[kIA1IRQ1].init("IA1IRQ1", 0x0B21, 15, 91, 1, false, 12);
  mTrapRegisters[kIA1IRQ2].init("IA1IRQ2", 0x0B22, 15, 91, 2, false, 12);
  mTrapRegisters[kIA1IRQ3].init("IA1IRQ3", 0x0B23, 15, 91, 3, false, 12);
  mTrapRegisters[kIA1IRQ4].init("IA1IRQ4", 0x0B24, 15, 91, 4, false, 12);
  mTrapRegisters[kIA1IRQ5].init("IA1IRQ5", 0x0B25, 15, 91, 5, false, 12);
  mTrapRegisters[kIA1IRQ6].init("IA1IRQ6", 0x0B26, 15, 91, 6, false, 12);
  mTrapRegisters[kIA1IRQ7].init("IA1IRQ7", 0x0B27, 15, 91, 7, false, 12);
  mTrapRegisters[kIA1IRQ8].init("IA1IRQ8", 0x0B28, 15, 91, 8, false, 12);
  mTrapRegisters[kIA1IRQ9].init("IA1IRQ9", 0x0B29, 15, 91, 9, false, 12);
  mTrapRegisters[kIA1IRQA].init("IA1IRQA", 0x0B2A, 15, 91, 10, false, 12);
  mTrapRegisters[kIA1IRQB].init("IA1IRQB", 0x0B2B, 15, 91, 11, false, 12);
  mTrapRegisters[kIA1IRQC].init("IA1IRQC", 0x0B2C, 15, 91, 12, false, 12);
  mTrapRegisters[kIRQSW1].init("IRQSW1", 0x0B2D, 15, 91, 13, false, 13);
  mTrapRegisters[kIRQHW1].init("IRQHW1", 0x0B2E, 15, 91, 14, false, 13);
  mTrapRegisters[kIRQHL1].init("IRQHL1", 0x0B2F, 15, 91, 15, false, 13);
  mTrapRegisters[kIA2IRQ0].init("IA2IRQ0", 0x0B40, 15, 99, 0, false, 12);
  mTrapRegisters[kIA2IRQ1].init("IA2IRQ1", 0x0B41, 15, 99, 1, false, 12);
  mTrapRegisters[kIA2IRQ2].init("IA2IRQ2", 0x0B42, 15, 99, 2, false, 12);
  mTrapRegisters[kIA2IRQ3].init("IA2IRQ3", 0x0B43, 15, 99, 3, false, 12);
  mTrapRegisters[kIA2IRQ4].init("IA2IRQ4", 0x0B44, 15, 99, 4, false, 12);
  mTrapRegisters[kIA2IRQ5].init("IA2IRQ5", 0x0B45, 15, 99, 5, false, 12);
  mTrapRegisters[kIA2IRQ6].init("IA2IRQ6", 0x0B46, 15, 99, 6, false, 12);
  mTrapRegisters[kIA2IRQ7].init("IA2IRQ7", 0x0B47, 15, 99, 7, false, 12);
  mTrapRegisters[kIA2IRQ8].init("IA2IRQ8", 0x0B48, 15, 99, 8, false, 12);
  mTrapRegisters[kIA2IRQ9].init("IA2IRQ9", 0x0B49, 15, 99, 9, false, 12);
  mTrapRegisters[kIA2IRQA].init("IA2IRQA", 0x0B4A, 15, 99, 10, false, 12);
  mTrapRegisters[kIA2IRQB].init("IA2IRQB", 0x0B4B, 15, 99, 11, false, 12);
  mTrapRegisters[kIA2IRQC].init("IA2IRQC", 0x0B4C, 15, 99, 12, false, 12);
  mTrapRegisters[kIRQSW2].init("IRQSW2", 0x0B4D, 15, 99, 13, false, 13);
  mTrapRegisters[kIRQHW2].init("IRQHW2", 0x0B4E, 15, 99, 14, false, 13);
  mTrapRegisters[kIRQHL2].init("IRQHL2", 0x0B4F, 15, 99, 15, false, 13);
  mTrapRegisters[kIA3IRQ0].init("IA3IRQ0", 0x0B60, 15, 107, 0, false, 12);
  mTrapRegisters[kIA3IRQ1].init("IA3IRQ1", 0x0B61, 15, 107, 1, false, 12);
  mTrapRegisters[kIA3IRQ2].init("IA3IRQ2", 0x0B62, 15, 107, 2, false, 12);
  mTrapRegisters[kIA3IRQ3].init("IA3IRQ3", 0x0B63, 15, 107, 3, false, 12);
  mTrapRegisters[kIA3IRQ4].init("IA3IRQ4", 0x0B64, 15, 107, 4, false, 12);
  mTrapRegisters[kIA3IRQ5].init("IA3IRQ5", 0x0B65, 15, 107, 5, false, 12);
  mTrapRegisters[kIA3IRQ6].init("IA3IRQ6", 0x0B66, 15, 107, 6, false, 12);
  mTrapRegisters[kIA3IRQ7].init("IA3IRQ7", 0x0B67, 15, 107, 7, false, 12);
  mTrapRegisters[kIA3IRQ8].init("IA3IRQ8", 0x0B68, 15, 107, 8, false, 12);
  mTrapRegisters[kIA3IRQ9].init("IA3IRQ9", 0x0B69, 15, 107, 9, false, 12);
  mTrapRegisters[kIA3IRQA].init("IA3IRQA", 0x0B6A, 15, 107, 10, false, 12);
  mTrapRegisters[kIA3IRQB].init("IA3IRQB", 0x0B6B, 15, 107, 11, false, 12);
  mTrapRegisters[kIA3IRQC].init("IA3IRQC", 0x0B6C, 15, 107, 12, false, 12);
  mTrapRegisters[kIRQSW3].init("IRQSW3", 0x0B6D, 15, 107, 13, false, 13);
  mTrapRegisters[kIRQHW3].init("IRQHW3", 0x0B6E, 15, 107, 14, false, 13);
  mTrapRegisters[kIRQHL3].init("IRQHL3", 0x0B6F, 15, 107, 15, false, 13);
  ...
  mTrapRegisters[kC08CPU0].init("C08CPU0", 0x0C00, 32, 126, 0, false, 32);
  mTrapRegisters[kC09CPU0].init("C09CPU0", 0x0C01, 32, 127, 0, false, 32);
  mTrapRegisters[kC10CPU0].init("C10CPU0", 0x0C02, 32, 128, 0, false, 32);
  mTrapRegisters[kC11CPU0].init("C11CPU0", 0x0C03, 32, 129, 0, false, 32);
  mTrapRegisters[kC12CPUA].init("C12CPUA", 0x0C04, 32, 130, 0, false, 32);
  mTrapRegisters[kC13CPUA].init("C13CPUA", 0x0C05, 32, 131, 0, false, 32);
  mTrapRegisters[kC14CPUA].init("C14CPUA", 0x0C06, 32, 132, 0, false, 32);
  mTrapRegisters[kC15CPUA].init("C15CPUA", 0x0C07, 32, 133, 0, false, 32);
  mTrapRegisters[kC08CPU1].init("C08CPU1", 0x0C08, 32, 134, 0, false, 32);
  mTrapRegisters[kC09CPU1].init("C09CPU1", 0x0C09, 32, 135, 0, false, 32);
  mTrapRegisters[kC10CPU1].init("C10CPU1", 0x0C0A, 32, 136, 0, false, 32);
  mTrapRegisters[kC11CPU1].init("C11CPU1", 0x0C0B, 32, 137, 0, false, 32);
  mTrapRegisters[kC08CPU2].init("C08CPU2", 0x0C10, 32, 138, 0, false, 32);
  mTrapRegisters[kC09CPU2].init("C09CPU2", 0x0C11, 32, 139, 0, false, 32);
  mTrapRegisters[kC10CPU2].init("C10CPU2", 0x0C12, 32, 140, 0, false, 32);
  mTrapRegisters[kC11CPU2].init("C11CPU2", 0x0C13, 32, 141, 0, false, 32);
  mTrapRegisters[kC08CPU3].init("C08CPU3", 0x0C18, 32, 142, 0, false, 32);
  mTrapRegisters[kC09CPU3].init("C09CPU3", 0x0C19, 32, 143, 0, false, 32);
  mTrapRegisters[kC10CPU3].init("C10CPU3", 0x0C1A, 32, 144, 0, false, 32);
  mTrapRegisters[kC11CPU3].init("C11CPU3", 0x0C1B, 32, 145, 0, false, 32);
...
uint32_t TrapConfig3::setRegisterValueByIdx(uint32_t data, uint32_t regidx, int mcmidx)
{
...
uint32_t TrapConfig3::setRegisterValueByAddr(uint32_t data, uint32_t addr, int mcmidx)
{
  int index = getRegIndexByAddr(addr);
...
uint32_t TrapConfig3::setRegisterValueByName(uint32_t data, const std::string& regname, int mcmidx)
{
  int index = getRegIndexByName(regname);
  if (index < 0) {
    LOGP(warn, "register name {} is invalid in {}", regname, __func__);
    return 0;
  }
  return setRegisterValueByIdx(data, index, mcmidx);
}
...
// the following are all unpacked values.
...
void TrapRegInfo::init(const std::string& name, int addr, int nbits, int base, int wordoffset, bool ignorechange, uint32_t max)
{
  ...
  if (addr != 0) {
    ...
    mDataWordNumber = wordoffset * (nbits + gapbits) / packedwordsize;  // the 32 bit word offset to the word containing the register in question
