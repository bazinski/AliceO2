...
uint32_t getChargeFromRawHeaders(const o2::trd::TrackletHCHeader& hcheader, const o2::trd::TrackletMCMHeader* header, const std::array<o2::trd::TrackletMCMData, 3>& data, int pidindex, int trackletindex)
{
  ...
  switch (datatype) {
    case 0: //Cosmic
      LOG(error) << "This is a problem cosmic format tracklets ";
      break;
    case 1: //TPT
      LOG(error) << "This is a problem  TPT format tracklets ";
      break;
    case 2: //DIS
      LOG(error) << "This is a problem  DIS format tracklets ";
      break;
    case 3:
      //PID VERSION 1
      ...
      uint32_t pidoffset = (pidword >> 18) & 0x3; // used for dynamic ranged charge windows.
...
bool sanityCheckTrackletHCHeader(o2::trd::TrackletHCHeader& header)
{
  ...
  if (dynamicqrange) {
    //tracklet charge format is 2 and we dont support that yet.
    goodheader = false;
  }
  return goodheader;
}
