...
class TrapRegInfo
{
 ...
 public:
  ...
  void init(const std::string& name, int addr, int nBits, int base, int wordoffset, bool ignorechange, uint32_t mask);
...
class TrapConfig3
{
 ...
 public:
  ...
  // get the register valuehe register index
  ...
  // get the registerbased on the register address
  uint32_t getRegisterValueByIdx(uint32_t regix, int sector, int stack, int layer, int rob, int mcm);    // get the registerbasd on the register index
  uint32_t getRegisterValueByAddr(uint32_t regaddr, int sector, int stack, int layer, int rob, int mcm); // get the registerbasd on the address of the register;
  uint32_t getRegisterValueByIdx(uint32_t regix, int detector, int rob, int mcm);                        // get the registerbasd on the register index
  uint32_t getRegisterValueByAddr(uint32_t regaddr, int detector, int rob, int mcm);                     // get the registerbasd on the address of the register;
  ...
  // retrieve the trap register info for a particular register, no need to index into det/rob/mcm as these are constant
  ...
  void getAllRegistersPacked(int mcmidx, std::array<uint32_t, kTrapRegistersSize>& mcmregisters);              // return all the registers for a particular mcm
  void getAllRegistersUnPacked(int mcmidx, std::array<uint32_t, kLastReg>& registers);                         // return all the registers for a particular mcm
 ...
 private:
  ...
  std::array<uint16_t, 0xe000> mTrapRegistersAddressIndex;                                // index by address into mTrapRegisters.
